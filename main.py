...
async def reporte_recibos(
    ...
    fname = f"recibos_{desde}-{hasta}" + (
        f"_{contribuyente.strip().upper().replace(' ', '_')}" if contribuyente else "") + ".pdf"
...
async def reporte_cedulas(
    ...
    headers = ["Folio", "Fecha", "Folio Elec.", "Contribuyente", "Motivo", "Dirección", "Importe", "Recibo",
               "Fecha Recibo"]
    rows = []
    for r in data:
